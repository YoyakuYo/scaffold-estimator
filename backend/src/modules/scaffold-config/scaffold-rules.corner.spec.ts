...
describe('fitSpansToWallLengthWithCorner (kusabi)', () => {
  it('uses 1800 first, 900+600 terminal, exact total for e.g. 10,200mm wall', () => {
    const wallMm = 10_200;
    const spans = fitSpansToWallLengthWithCorner(wallMm);
    expect(spans[0]).toBe(CORNER_START_SPAN_MM);
    expect(spans[spans.length - 2]).toBe(CORNER_APPROACH_SPAN_MM);
  ...
  it('exact three-span layout when wallLength = 3000mm (no middle)', () => {
    const spans = fitSpansToWallLengthWithCorner(3000);
  ...
  it('falls back to 600–600 pattern when wall is too short for 1800+900+600+overrun', () => {
...
describe('fitSpansToWallLengthWithCornerWakugumi', () => {
  it('uses 1829 first and 914+610 at end; total run >= wall+300', () => {
    const wallMm = 10_000;
    const spans = fitSpansToWallLengthWithCornerWakugumi(wallMm);
    expect(spans[0]).toBe(WAKUGUMI_CORNER_START_SPAN_MM);
    expect(spans[spans.length - 2]).toBe(WAKUGUMI_CORNER_APPROACH_SPAN_MM);
  ...
  it('exact three-span when wallLength fits 1829+914+610+300', () => {
    const wallMm =
      WAKUGUMI_CORNER_START_SPAN_MM +
      WAKUGUMI_CORNER_APPROACH_SPAN_MM +
      WAKUGUMI_CORNER_SPAN_MM -
