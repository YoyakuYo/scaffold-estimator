...
describe('fitSpansToWallLengthWithCorner (kusabi)', () => {
  ...
  it('exact two-span layout when wallLength = 2400mm (no middle)', () => {
    const spans = fitSpansToWallLengthWithCorner(2400);
    expect(spans).toEqual([CORNER_START_SPAN_MM, CORNER_TERMINAL_SPAN_MM]);
  });

  it('falls back to 600–600 pattern when wall is too short for 1800+900+overrun', () => {
...
describe('fitSpansToWallLengthWithCornerWakugumi', () => {
  it('uses 1829 first, single 914 terminal; total run >= wall+300', () => {
    const wallMm = 10_000;
    const spans = fitSpansToWallLengthWithCornerWakugumi(wallMm);
    expect(spans[0]).toBe(WAKUGUMI_CORNER_START_SPAN_MM);
    expect(spans[spans.length - 1]).toBe(WAKUGUMI_CORNER_TERMINAL_SPAN_MM);
    const sum = spans.reduce((a, b) => a + b, 0);
  ...
  it('exact two-span when wallLength fits 1829+914+300', () => {
    const wallMm =
      WAKUGUMI_CORNER_START_SPAN_MM +
      WAKUGUMI_CORNER_TERMINAL_SPAN_MM -
      WAKUGUMI_CORNER_OVERRUN_MM;
