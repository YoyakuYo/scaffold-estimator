...
export class ScaffoldCalculatorService {
  ...
  private calculateWall(
    ...
    const jackBaseQty = Math.max(0, postPositions * 2 + totalExtraPosts - cornerPostDeduction);
    sortOrder++;
    components.push({
      type: 'jack_base',
    ...
    for (const [spanSizeMm, count] of Object.entries(spanGroups)) {
      ...
      nunoBarsBySize[size].tesuri += Number(count) * Lsafety * CALC_RULES.tesuriPerSpanPerLevel;
    }
    ...
    // ─── 相間ブラゲット / 装間ネット / メッシュシート（内列支柱・スパン長ベース）──
    const innerPostStations = Math.max(0, postPositions - Math.floor(cornerPostDeduction / 2));
