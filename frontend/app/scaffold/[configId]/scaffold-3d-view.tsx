  ...
  useEffect(() => {
    ...
    import('three').then(async (THREE) => {
          ...
          for (let i = startSpanIdx; i < spans.length; i++) {
            ...
            if (!isDoorSpan) {
              ...
              if (isWakugumiTopGuardLift) {
                ...
                const braceZ = isBracket ? 0 : tzOuter;
            ...
            const showPlankHere =
              (!isStairSpan || needsExtendedBay) && !isDoorSpan && !guardLiftNoDeck;
            if (showPlankHere) {
            ...
            // Skip habaki at ground level for door openings; 枠組最上ガード段は踏板なしのため巾木も出さない。
            if (!isDoorSpan && !guardLiftNoDeck) {
      ...
      for (let i = 0; i < walls.length; i++) {
        ...
        const totalH = baseYM + GROUND_Y + JACK_H + levelsShown * LEVEL_H + (levelsShown >= levels ? sceneTopExtensionM : 0);
        ...
        for (let localWi = 0; localWi < tg.walls.length; localWi++) {
          ...
          if (maxLvThisCorner > 0) {
            ...
            for (const { pt, isOuter } of cornerPostsMeta) {
              ...
              addPipe(cornerGroup, pt.x, GROUND_Y, pt.z, pt.x, GROUND_Y + JACK_H, pt.z, jackMatEff, PIPE_R * 0.95);
