    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="overflow-auto" style={{ maxHeight: '700px' }}>
        ...
        >
          ...
          {/* Corner patches — two overlapping rectangles per 足場コーナー詳細図 */}
          {isClosed && edges.length >= 2 && edges.map((_, wi) => {
            ...
            // Edge directions
            const eBdx = (eB.x2 - eB.x1) / lenB;
            const eBdy = (eB.y2 - eB.y1) / lenB;

            const sw = SCAFFOLD_STRIP_W;
            const vx = eA.x2 + offsetX;
            const vy = eA.y2 + offsetY;

            // Rectangle 1: wall A strip extended along wall B direction
            const a1x = vx + nAx * sw, a1y = vy + nAy * sw;
            const a2x = a1x + eBdx * sw, a2y = a1y + eBdy * sw;
            const a3x = vx + eBdx * sw, a3y = vy + eBdy * sw;

            // Rectangle 2: wall B strip extended along wall A direction (opposite of eA)
            const eAdx = (eA.x2 - eA.x1) / lenA;
            const eAdy = (eA.y2 - eA.y1) / lenA;
            const b1x = vx + nBx * sw, b1y = vy + nBy * sw;
            const b2x = b1x + eAdx * sw, b2y = b1y + eAdy * sw;
            const b3x = vx + eAdx * sw, b3y = vy + eAdy * sw;

            return (
              <g key={`corner-${wi}`}>
                {/* Wall A extension */}
                <polygon
                  points={`${vx},${vy} ${a1x},${a1y} ${a2x},${a2y} ${a3x},${a3y}`}
                  fill={colA.fill} stroke={colA.stroke} strokeWidth={1} opacity={0.5}
                />
                {/* Wall B extension */}
                <polygon
                  points={`${vx},${vy} ${b1x},${b1y} ${b2x},${b2y} ${b3x},${b3y}`}
                  fill={colB.fill} stroke={colB.stroke} strokeWidth={1} opacity={0.5}
                />
                {/* Post markers */}
                <circle cx={vx} cy={vy} r={2.5} fill="#334155" />
                <circle cx={a2x} cy={a2y} r={2.5} fill={colA.stroke} />
                <circle cx={b2x} cy={b2y} r={2.5} fill={colB.stroke} />
                <circle cx={a1x} cy={a1y} r={2} fill={colA.stroke} opacity={0.7} />
                <circle cx={b1x} cy={b1y} r={2} fill={colB.stroke} opacity={0.7} />
              </g>
            );
          })}
